...
if(gameState===2){
    ...
    for(var i=400; i<1200; i=i+60){
      image(goblinImg, i, 60, 45, 45);
...
//player2 movements
    ...
    trophy = createSprite(1360, 350, 40, 40);
    trophy.addImage("trophy", trophyImg);
    trophy.scale = 0.5;
    
    if(player1.body.isTouching(trophy)){
        gameState=3;
        winner = "player1";
    }

    if(player2.body.isTouching(trophy)){
      gameState=3;
      winner = "player2"
  }
  ...
  if(gameState===3){
    if(winner === "player1"){
      ...
      fill("red")
      stroke("white");
      text("PLAYER1 IS THE WINNER", 400,300);
 }
 ...
 if(winner === "player2"){
  ...
  fill("red")
  stroke("white");
  text("PLAYER2 IS THE WINNER", 400,300);
}
}
}
}